...
export class MicroPandaTestRunner implements vscode.Disposable {
    ...
    constructor(private readonly diagnostics: MicroPandaDiagnostics) {
        ...
        this.watcher = vscode.workspace.createFileSystemWatcher('**/*_test.mpd');
        ...
        vscode.workspace.findFiles('**/*_test.mpd').then(uris => {
    ...
    private refreshFile(uri: vscode.Uri) {
        ...
        const label = path.basename(uri.fsPath, '.mpd');
    ...
    private runFile(
        ...
        return new Promise(resolve => {
            ...
            proc.on('error', err => {
                const msg = new vscode.TestMessage(
                    `Failed to launch '${mpdExe}': ${err.message}. ` +
                    `Check the 'microPanda.mpdPath' setting.`
                );
                for (const item of items) { run.errored(item, msg); }
                resolve();
        ...
        while (i < lines.length) {
            ...
            if (pm) {
                ...
                while (i < lines.length && lines[i].startsWith('  ')) {
                    ...
                    // Format: /abs/path/file.mpd:<line>: <assert expression>
                    const dm = detail.match(/^(.+\.mpd):(\d+): (.+)$/);
                    if (dm) {
